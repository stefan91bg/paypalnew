...
    // ✅ ISPRAVKA 1: Robusno pronalaženje URL-ova
    const { workspaceId, reportsUrl, backendUrl } = decodedToken;
    const safeBackendUrl =
      backendUrl ||
      decodedToken.apiUrl ||
      decodedToken.backend ||
      null;
    const safeReportsUrl =
      reportsUrl ||
      decodedToken.reportsUrl ||
      (safeBackendUrl ? safeBackendUrl.replace(/\/api$/, '') + '/reports' : null);

    if (!safeBackendUrl || !safeReportsUrl) {
      throw new Error("Missing environment URL parameters in token");
    }

    // ✅ ISPRAVKA 2: Defanzivno prihvatanje vrednosti iz 'req.body'
    const { 
      start, end, billableFilter, projectFilter, clientFilter, 
      taskFilter, descriptionFilter, withoutTask, withoutDescription, 
      preview, 
      columns: visibleColumns = { date: true, description: true, project: true, task: true } 
    } = req.body;

    // Postavljanje 'fallback' vrednosti sprečava padanje PDF generatora
    const USER_PAYPAL_LINK = req.body.USER_PAYPAL_LINK || "";
...
    const workspaceResp = await fetch(`${safeBackendUrl}/v1/workspaces/${workspaceId}`, { headers: { "X-Addon-Token": token } });
...
    // ✅ ISPRAVKA 3: Provera 'descriptionFilter'
    if (withoutDescription) {
      clockifyPayload.withoutDescription = true;
    } else if (descriptionFilter) { 
      const trimmedDescription = descriptionFilter.trim();
      if (trimmedDescription !== '') {
        clockifyPayload.description = trimmedDescription;
      }
    }

    // ✅ Koristi se 'safeReportsUrl'
    const reportApiUrl = `${safeReportsUrl}/v1/workspaces/${workspaceId}/reports/detailed`;
...
  } catch (err) {
    console.error("API /reports error:", err.message, err.stack);
